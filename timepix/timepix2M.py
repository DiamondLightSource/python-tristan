...
def timepix_parser():
    parser = argparse.ArgumentParser(description = "Generates images from event-mode data.")
    parser.add_argument("event_data", nargs=1, help="Input event mode data from timepix detector, Nexus file")
    parser.add_argument("exposure_time", type=float, nargs="?", default=0.1, help="Image exposure time, float")
    parser.add_argument("step", type=int, nargs="?", default=100000, help="Because data is not chunked, define a step to load this many events at a time.")
    parser.add_argument("image_file", nargs=1, help= "Output file to save reconstructed images, HDF5 file")

    args = parser.parse_args()
    #TBD: make image_file optional, give warning and just not write file if nothing is apssed

    Timepix2MImageConverter(args.event_data, args.exposure_time, args.step, args.image_file).run()
...
class Timepix2MImageConverter(object):
    ...
    def __init__(self, event_data, exposure_time, step, image_file):
        ...
        assert len(self._ttl)>0, "No ttl_rising trigger messages found in either module."
    ...
    def _discover_shutter_times(self):
        ...
        assert (np.all(_open) == True) and (np.all(_close) == True), "Trigger signals do not match"
    ...
    def get_time_interval(self, ttl_up):
        " Get interval around laser pulse that we want to observe: 0.8s before pulse to 4s after."
    ...
    def make_histogram(self, xyt):
        ...
        img_range = np.arange(img_start, img_end, dtype=int)
